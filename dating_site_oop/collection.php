...
$data = $con->showCollection($phone);


if (isset($_POST['equip'])) {
    $avatar_img = $_POST['avatar_img'];

    $total = $con->updateAvatar($avatar_img, $phone);
    if ($total) {
        echo "<script>
        ...
        <div class="container">
            <div class="row py-5 justify-content-evenly border border-black">
                <?php foreach ($data as $row) : ?>
                    <form action="collection.php" method="POST" class="col-lg-3 ">
                        <div class="card my-2">

                            <img src="avatar/<?= $row['avatar_bought']; ?>" class="profile-img img-fluid mb-3" alt="" style="cursor: pointer;">
                            <div class="tombol d-flex justify-content-end mx-3 my-3">
                                <input type="hidden" name="avatar_img" class="pesan mb-3 " value="<?= $row['avatar_bought']; ?>">
                                <a id="gift" class="btn-like" href="wishgift.php?avatar_img=<?= $row["avatar_bought"]; ?>">
                                    <i class="fa-solid fa-gift fa-2x mx-3"></i>
                                </a>
                                <input type="submit" name="equip" class="pesan mb-3 " value="Equip">
                            </div>
